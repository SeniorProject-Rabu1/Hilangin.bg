...
class camshift_test(NewOpenCVTests):
    ...
    def prepareRender(self):
        ...
        self.render = tst_scene_render.TestSceneRender(self.get_sample("bgr.png"), deformation = True)
    ...
    def runTracker(self):
        ...
        while True:
            ...
            if intersectionRate(self.render.getCurrentRect(), trackingRect) < 2:
                self.errors += 1

            if framesCounter > self.framesNum:
                break

        self.assertLess(float(self.errors) / self.framesNum, 2)
